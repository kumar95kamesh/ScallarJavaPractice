...
/**
 * This is copy of sorter class.. 
 * Actually in sorter main class if we see we have an executor service.. which creates 2 thread pool
 * and in sorter class also we have another executor service and for each loop we are creating 2 new
 * thread pool.. so better we make it as parameterized so in the main method we will start only one
 * executor service.
 * 
 */
public class P2_SorterCopy implements Callable<List<Integer>> {//what data type will return callable
	List<Integer> arrayToSort;
	ExecutorService es;
	
	public P2_SorterCopy(List<Integer> arrayToSort,ExecutorService es) {//to feed the data.
		this.arrayToSort=arrayToSort;					//basically jb bhi object bnega y parameters provide krne hi pdenge.
		this.es=es;
	}
	...
	public List<Integer> call() throws Exception {//method from callable interface
		...
		P2_SorterCopy leftsorter=new P2_SorterCopy(leftArray,es);
		P2_SorterCopy rightsorter=new P2_SorterCopy(rightArray,es);
...
//		ExecutorService es=Executors.newFixedThreadPool(2);//we have two threads only so
		/**
		 * we don't need to start new executor service everytime.
		 */
